...
print_final_instructions() {
    ...
    echo ""
    echo "6. Drop in API keys for the non-Anthropic Claude Code backends you use"
    echo "   (claudeglm/claudeor/claudeds/claudeqwen, already sourced into your shell):"
    echo "┌──────────────────────────────────────────────────────────┐"
    echo "│ mkdir -p ~/.config/{zai,crush,qwen}                       │"
    echo "│ echo YOUR_KEY > ~/.config/zai/coding-key       # claudeglm │"
    echo "│ echo YOUR_KEY > ~/.config/crush/deepseek-key   # claudeds  │"
    echo "│ echo YOUR_KEY > ~/.config/crush/openrouter-key # claudeor  │"
    echo "│ echo YOUR_KEY > ~/.config/qwen/dashscope-key   # claudeqwen│"
    echo "│ chmod 600 ~/.config/{zai/coding-key,crush/deepseek-key,\\  │"
    echo "│   crush/openrouter-key,qwen/dashscope-key}                │"
    echo "└──────────────────────────────────────────────────────────┘"
    echo "   Skip any you don't use. Restart your shell (or source the rc file)"
    echo "   afterwards so claudeglm/claudeor/claudeds/claudeqwen pick up the key."
    echo ""
